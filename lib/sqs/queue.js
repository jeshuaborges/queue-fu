...
var Queue = function() {
  ...
  this.poll = new Poll(this.fetchJobs.bind(this), this);
};
...
Queue.CONFIG_DEFAULTS = {
  interval: 0, // How long to wait between requests (ms).
  pollSeconds: 20, // How long to leave a request open (max: 20 seconds).
};
...
Queue.prototype.getMessages = function() {
  return this.sqs.receiveMessage({
    QueueUrl: this.config.queueUrl,
    WaitTimeSeconds: this.config.pollSeconds,
  });
};
